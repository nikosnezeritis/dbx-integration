...
```bash
conda create -n test_project python=3.9
...
```
This command will create your personal repository under `/Repos/<username>/test_project`.
3. To set up the CI/CD pipeline with the notebook, create a separate `Staging` repo:
```bash
databricks repos create --url <your repo URL> --provider <your-provider> --path /Repos/Staging/test_project
```
...
- To trigger the release pipeline, get the current version from the `test_project/__init__.py` file and tag the current code version:
```
